...
of \S\ref{sec:consensus}; every node computes the identical winner from the same chain state. In
integration the validator's auto-submitted storage proofs (\S\ref{sec:audit}) are accepted on-chain
and raise its score, which the weighting then reads, so block-production rights track proven storage
by construction in the deployed binary; observing this accrual on the public network is gated only
on rolling the upgrade out to validators, not on any missing mechanism.
