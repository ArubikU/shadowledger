...
of \S\ref{sec:consensus}; every node computes the identical winner from the same chain state. End to
end, a validator's auto-submitted storage proofs (\S\ref{sec:audit}) are accepted on-chain, raise
its score, and shift its election share, so block-production rights track proven storage in the
running system rather than only in design.
