...
holding-but-withholding economically self-defeating. A concrete hardening, which the HRW layer
admits without protocol change, is \emph{anti-affinity placement}: rank holders not over raw node
identities but over (operator-or-ASN, identity) pairs declared at registration, so the top-$r$
holders of a shard fall in distinct failure domains by construction; an operator controlling a
fraction $\phi$ of declared domains then holds at most $\lceil r\phi\rceil$ of any shard's copies,
and tolerating a fully-correlated domain of that size needs only $r>1/(1-\phi)$ rather than trusting
independence. This shifts the assumption from ``failures are independent'' to ``no single domain
exceeds $\phi$,'' which is auditable. We implement identity-based HRW today and flag domain-aware
placement as the concrete next mitigation, stating the correlation risk as a real limitation rather
