...
func TestTokenContractEndToEnd(t *testing.T) {
	...
	bal := func(dig uint64) uint64 {
		r, _ := s.QueryContract(tok, 0, words(shl.Selector("balanceOf"), dig), 0)
		return r
	}

	// init: dev gets the whole supply (dev nonce 1 — nonce 0 was the deploy).
	call(dev, 1, shl.Selector("init"))
	if bal(devDig) != 1_000_000 {
		t.Fatalf("after init dev balance = %d", bal(devDig))
	}
	if total, _ := s.QueryContract(tok, 0, words(shl.Selector("supply")), 0); total != 1_000_000 {
		t.Fatalf("totalSupply = %d", total)
	}

	// transfer 250k dev -> bob.
	call(dev, 2, shl.Selector("transfer"), bobDig, 250_000)
	if bal(devDig) != 750_000 || bal(bobDig) != 250_000 {
		t.Fatalf("after transfer dev=%d bob=%d", bal(devDig), bal(bobDig))
	}

	// overdraw: bob tries to send 999k he doesn't have -> require reverts (no change).
	call(bob, 0, shl.Selector("transfer"), devDig, 999_000) // bob's first tx, nonce 0
...
func TestMemecoinBurn(t *testing.T) {
	...
	total := func() uint64 { r, _ := s.QueryContract(tok, 0, words(shl.Selector("supply")), 0); return r }
	bal := func() uint64 { r, _ := s.QueryContract(tok, 0, words(shl.Selector("balanceOf"), devDig), 0); return r }

	call(1, shl.Selector("init"))          // init -> 21,000,000 (nonce 1; nonce 0 was the deploy)
	call(2, shl.Selector("burn"), 1000000) // burn 1,000,000
